...
import { getOperations, deleteOperationById } from 'apiSdk/operations';
import { OperationInterface } from 'interfaces/operation';

type ColumnType = ColumnDef<OperationInterface, unknown>;
...
export function OperationListPage(props: OperationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getOperations({
        ...
        order: params.order,
        searchTermKeys: ['operation_type.contains', 'operation_status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<OperationInterface>>(
    () => `/operations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: OperationInterface) => {
    if (hasAccess('operation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/operations/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'operation_type', header: 'Operation Type', accessorKey: 'operation_type' },
    { id: 'operation_status', header: 'Operation Status', accessorKey: 'operation_status' },
    {
      id: 'start_time',
      header: 'Start Time',
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              ...
              {hasAccess('operation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
              ...
              {hasAccess('operation', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="operation" />
        </Flex>

        {hasAccess('operation', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
