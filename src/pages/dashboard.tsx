...
import { getOperations } from 'apiSdk/operations';
import { OperationInterface } from 'interfaces/operation';

import { OperationListPage as ShowCaseList } from 'pages/operations';
...
function DashboardPage() {
  ...
  const operationStat = useSWR<PaginatedInterface<OperationInterface>>(
    () => `/operations`,
    () => getOperations({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Operations',
      result: operationStat?.data?.totalCount,
      isLoading: operationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`operations`} passHref legacyBehavior>
                  <Button
